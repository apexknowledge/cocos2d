...
public class CCLabelAtlas extends CCAtlasNode implements CCLabelProtocol, CCNode.CocosNodeSize {
    ...
    /**
     * creates the CCLabelAtlas with a string, a char map file(the atlas), the width and height of
     * each element and the starting char of the atlas
     */
    public static CCLabelAtlas label(CharSequence theString, String charmapfile, int w, int h,
            char c) {
        return new CCLabelAtlas(theString, charmapfile, w, h, c);
    }

    /**
     * initializes the CCLabelAtlas with a string, a char map file(the atlas), the width and height
     * of each element and the starting char of the atlas
     */
    ...
    public void draw(GL10 gl) {
        ...
        /*
         * if (ccConfig.CC_LABELATLAS_DEBUG_DRAW) { CGSize s = [self contentSize]; CGPoint
         * vertices[4]={ ccp(0,0),ccp(s.width,0), ccp(s.width,s.height),ccp(0,s.height), };
         * ccDrawPoly(vertices, 4, YES); } // CC_LABELATLAS_DEBUG_DRAW
         */
    }
